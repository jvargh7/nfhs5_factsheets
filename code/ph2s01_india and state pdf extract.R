...
library(tabulizer)
library(tidyverse)
nfhs5_phase2_data <- extract_tables(paste0(data_path,"/NFHS5 Factsheets/NFHS-5 India and State Factsheet Compendium_Phase-II.pdf"),
...
# 
# ML_111to131 <- extract_tables(paste0(data_path,"/NFHS5 Factsheets/NFHS-5 State Factsheet Compendium_Phase-II.pdf"),pages = 98,
...
nfhs5_phase2_data[[1]] <- NULL


nfhs5_tables <- map(nfhs5_phase2_data,.f=function(x){
  ...
  bind_rows(.) %>% 
  dplyr::filter(!dplyr::lag(Indicator)=="") %>% 
...
if(!dir.exists("phase 2 release/states extract")){
  dir.create("phase 2 release/states extract")
}
saveRDS(nfhs5_tables,"phase 2 release/states extract/ph201s_output.RDS")
